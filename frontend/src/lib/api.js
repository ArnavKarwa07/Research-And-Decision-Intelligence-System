...
export const api = {
  ...
  // Phase 13 Enterprise Connectors Endpoints (/api/v1/connectors/*)
  listConnectors: (params = {}) => {
    const search = new URLSearchParams();
    if (params.workspace_id) search.append('workspace_id', params.workspace_id);
    if (params.provider_type) search.append('provider_type', params.provider_type);
    const q = search.toString() ? `?${search.toString()}` : '';
    return fetchApi(`/connectors${q}`);
  },
  ...
  updateConnector: (id, data) =>
    fetchApi(`/connectors/${id}`, {
      method: 'PATCH',
      body: JSON.stringify(data),
    }),

  triggerConnectorSync: (id, jobType = 'FULL_SYNC') =>
    fetchApi(`/connectors/${id}/sync`, {
      method: 'POST',
      body: JSON.stringify({ job_type: jobType }),
    }),

  getConnectorHealth: (id) => fetchApi(`/connectors/${id}/health`),

  // Phase 13 SSO Auth Endpoints (/api/v1/auth/*)
  initiateSSOLogin: (provider) =>
    fetchApi('/auth/sso/login', {
      method: 'POST',
      body: JSON.stringify({ provider }),
    }),

  ssoCallback: (provider, code) =>
    fetchApi('/auth/sso/callback', {
      method: 'POST',
      body: JSON.stringify({ provider, code }),
    }),

  logoutSession: () => fetchApi('/auth/logout', { method: 'POST' }),

  // Phase 13 Workspaces & RBAC Endpoints (/api/v1/workspaces/*)
  listWorkspaces: (orgId = null) => {
    const q = orgId ? `?org_id=${orgId}` : '';
    return fetchApi(`/workspaces${q}`);
  },
  ...
  listWorkspaceMembers: (workspaceId) => fetchApi(`/workspaces/${workspaceId}/members`),
  ...
  // Phase 13 Governance & Audit Logs (/api/v1/governance/*)
  queryAuditLogs: (params = {}) => {
    const search = new URLSearchParams();
    if (params.org_id) search.append('org_id', params.org_id);
    if (params.workspace_id) search.append('workspace_id', params.workspace_id);
    if (params.category) search.append('category', params.category);
    if (params.severity) search.append('severity', params.severity);
    if (params.limit) search.append('limit', params.limit);
    const q = search.toString() ? `?${search.toString()}` : '';
    return fetchApi(`/governance/audit-logs${q}`);
  },

  generateGovernanceReport: (orgId = 'default-org') =>
    fetchApi(`/governance/audit-reports?org_id=${orgId}`, { method: 'POST' }),
};
